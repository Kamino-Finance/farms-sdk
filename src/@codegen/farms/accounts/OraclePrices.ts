...
export class OraclePrices {
  ...
  static async fetch(
    ...
    if (info.programAddress !== programId) {
      throw new Error(
        `OraclePricesFields account ${address} belongs to wrong program ${info.programAddress}, expected ${programId}`
      )
    }
  ...
  static async fetchMultiple(
    ...
    return infos.map((info) => {
      ...
      if (info.programAddress !== programId) {
        throw new Error(
          `OraclePricesFields account ${info.address} belongs to wrong program ${info.programAddress}, expected ${programId}`
        )
      }
  ...
  static decode(data: Uint8Array): OraclePrices {
    if (data.length < OraclePrices.discriminator.length) {
      throw new Error("invalid account discriminator")
    }
    for (let i = 0; i < OraclePrices.discriminator.length; i++) {
      if (data[i] !== OraclePrices.discriminator[i]) {
        throw new Error("invalid account discriminator")
      }
    }

    const dec = OraclePrices.layout.decode(
      data.subarray(OraclePrices.discriminator.length)
    )
